...
HS_Mohasebati=HS;
for j=1:NN
if Ho(j,1)==0% mige agar matris Ho ke ghablan fesharhae moshahedati ra dar oon gharar dadim agar deraee az an sefr bod byad feshar mohasebati ra dar HHo gharar bede ke ghablan sefr bod hamash
HHo(j,1)=HS(j,1);% mohasebatiha ra rikht dakhel HHO be joz draehaee ke moshahedati ra darim
end
...
for i=1:AN-1
   ...
   k=find(x(j)<=pc,1,'first');% myad shomare oon jaega ehtemal ra dar matris midahad
...
%kod bala myad dakhel debiha jstojo mikne va brae har daste debi ke
%entekhab mikne fesharha va morabe anha ra bayad mohasebe kard pas braye
%har daste dbi 4 ta matris darim 1- hmoon debiha 2- fesharha 3- moraba
%fesharha 4- majmooe morabaat barye har marhale

%%
%va dobare omade bar asas debehae ghabli ke bedast omade fesharha ra
%mohasebe karde

HSS=DEpanet1(DEMAND_haye_entekhabi_baraye_NN_NODE,NN);% in ek tabe ast
for j=1:NN
if HSS(1,j)<0
HSS(1,j)=0
end
...
while fitness(1,t-1) > fitness_e_morede_nazar(1,t-1) %nabayad in kamtar mosavi bashad?
for j=1:NN
for i=1:AN
TT(i,1)=T(i,j);
end
